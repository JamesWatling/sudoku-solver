...
export var rows = ['A','B','C','D','E','F','G','H','I'];
export var cols = ['1','2','3','4','5','6','7','8','9'];
var digits = "123456789";
export var squares = cross(rows, cols);
var nassigns = 0;
var neliminations = 0;
var nsearches = 0;
...
export function parse_grid(grid){ // Given a string of 81 digits (or . or 0 or -), return an object os {cell:values}
  nassigns = 0;
  neliminations = 0;
  nsearches = 0;
  var grid2 = "";
...
export function assign(values, sq, dig){ // Eliminate all the other values (except dig) from values[sq] and propagate.
  ++nassigns;
  var result = true;
  var vals = values[sq];
...
export function eliminate(values, sq, dig){
  ++neliminations;
...
export function board_string(values){ // Used for debugging
  var width = 0;
  for (var s in squares)
